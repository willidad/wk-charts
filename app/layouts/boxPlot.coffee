...
angular.module('wk.chart').directive 'boxPlot', ($log, utils, barConfig, dataManagerFactory, markerFactory, tooltipHelperFactory)->
  ...
  link: (scope, element, attrs, controller) ->
    ...
    drawPath = (doAnimate, data, options, x, y, color) ->
      ...
      q = box.select('.wk-chart-box-rect')
      (if doAnimate then q.transition().duration( options.duration) else q)
        .attr('y', (d) -> y.scale()(d.uq))
        .attr('height', (d) -> Math.abs(y.scale()(d.uq) - y.scale()(d.lq)))
        .attr('width', (d) -> if d.added or d.deleted then 0 else barWidth)
        .style('opacity', 1).style('stroke', 'black')

      median = box.select('.wk-chart-box-med')
      (if doAnimate then median.transition().duration( options.duration) else median)
        .attr('x1', 0)
        .attr('x2', (d) -> if d.added or d.deleted then 0 else barWidth)
        .attr('y1', (d) -> y.scale()(d.med))
        .attr('y2', (d) -> y.scale()(d.med))
        .style('opacity', 1)
      ...
      (if doAnimate then lWhisker.transition().duration( options.duration) else lWhisker)
        ...
        .style('opacity', (d) -> if showMin then 1 else 0)
      ...
      (if doAnimate then lwConn.transition().duration( options.duration) else lwConn)
        ...
        .attr('y2', (d) -> if showMin then y.scale()(d.min) else 0)
        .style('opacity', if showMin then 1 else 0)
      ...
      (if doAnimate then uWhisker.transition().duration( options.duration) else uWhisker)
        ...
        .style('opacity', if showMax then 1 else 0)
